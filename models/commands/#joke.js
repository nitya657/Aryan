...
module.exports.run = async function({ event, api, args, client, Currencies, Users, utils, __GLOBAL, reminder }) {
    ...
    // Joke ka array
    const jokeList = ["Attitude 😎 दिखाना 😁मेरी 🤗 एक बिमारी है…. और इसे 🤐 ठीक👍करने के 😜 लिए मुझे 🤘 जरुरत तुम्हारी है💯💯",
    ...
    // Joke aur profile picture ke saath message bhejne ka function
    const sendjokeWithProfilePic = (joke, picture) => {
        api.sendMessage({
            body: joke,
    ...
    const sendProfilePic = (uid, joke) => {
        const callback = () => sendJokeWithProfilePic(joke, __dirname + "/cache/1.png");
    ...
    if (event.name == "joke" && args[0] == "credits") {
        sendWarningMessage();
    }
};
